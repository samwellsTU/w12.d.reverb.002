...
outputGain.gain.value = 1.0; // Set output volume to 50% -6dBFS
...
const createReverb = async function () {
  ...
  let response = await fetch("ir/chopin-rev.wav");
  let arraybuf = await response.arrayBuffer();
  convolver.buffer = await audioContext.decodeAudioData(arraybuf);
